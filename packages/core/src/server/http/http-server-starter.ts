 ...
 */
import { AppProps, propsStore } from '#ustra/core/config'
import { configLoader } from '#ustra/core/config/loader/config-loader'
import { createServer, Server, RequestListener, ServerOptions } from 'node:http'
import { useLogger } from '#ustra/core'
import coreUtils from '#ustra/core/utils/core'
import functionUtils from '#ustra/core/utils/functions'
 ...
 * 서버 구동 옵션
 */
export interface ServerRunningOption<App extends RequestListener = RequestListener> {
  /**
   * 어플리케이션
   */
  app: App
   ...
   */
  afterMiddlewareHandler?: (app: App, appPros: AppProps) => void | Promise<void>
}

/**
 * Http 서버 구동 추상 클래스
 */
export abstract class HttpServerStarter<App extends RequestListener = RequestListener> {
  /**
  ...
  async run(options: ServerRunningOption<App>): Promise<Server> {
    ...
    if (options.applyMiddlewares) {
      ...
      if (options.afterMiddlewareHandler) {
        const handlerResult = options.afterMiddlewareHandler(options.app, options.appProps)
