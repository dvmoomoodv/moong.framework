...
import { AppProps } from '#ustra/core/config'
import coreUtils from '#ustra/core/utils/core'
import arrayUtils from '#ustra/core/utils/array'
import pathUtils from '#ustra/core/utils/path'
import { logger } from '#ustra/core/utils/logging'
...
export class MiddlewareUtils {
   ...
   * @returns app
   */
  applyNodeMiddleWareToApp<Option extends NodeMiddleWareOption = NodeMiddleWareOption>(
    app: any,
    option: Option,
    applyMiddlwareHandler: (req: any, res: any, next: any) => void | Promise<void>,
  ) {
    if (!option.enabled) {
      return app
    }
  ...
  private toNodeMiddleware<Option extends NodeMiddleWareOption = NodeMiddleWareOption>(
    option: Option,
    applyMiddlwareHandler: (req: any, res: any, next: any) => void | Promise<void>,
  ) {
    if (!option.enabled) {
  ...
  setupMiddlewares(app: any, appProps: AppProps) {
    ...
    this.generateMiddlewares(appProps).forEach(middleware => app.use(middleware))
  }
  ...
  private createProxyMiddleware(appProps: AppProps) {
    ...
    Object.keys(appProps.server.middleware.proxy.proxies).forEach(c => {
      ...
      if (typeof opts === 'object') {
        opts = coreUtils.assign({ changeOrigin: true, rewrite: false, configure: false, bypass: false }, opts) as ProxyOptions
      }
          ...
          if (doesProxyContextMatchUrl(context, url)) {
            ...
            if (opts.bypass) {
              ...
              if (typeof bypassResult === 'string') {
                ...
                console.debug('bypass: ' + req.url + ' use modified options: %O', options)
                return next()
  ...
  generateMiddlewares(appProps: AppProps, useRequestContext = true) {
    ...
    if (appProps.server.middleware.compression.enabled) {
      const compressionMiddleware = compression(appProps.server.middleware.compression)
      middlewares.push(
        this.toNodeMiddleware(appProps.server.middleware.compression, (req, res, next) => {
    ...
    if (appProps.server.middleware.logging.enabled) {
      const loggingMiddleware = morgan(appProps.server.middleware.logging.format, appProps.server.middleware.logging.options)
      middlewares.push(this.toNodeMiddleware(appProps.server.middleware.logging, (req, res, next) => loggingMiddleware(req, res, next)))
    }

    // body parsers
    if (appProps.server.middleware.jsonBodyParser.enabled) {
      const jsonMiddleware = json(appProps.server.middleware.jsonBodyParser)
      middlewares.push(this.toNodeMiddleware(appProps.server.middleware.jsonBodyParser, (req, res, next) => jsonMiddleware(req, res, next)))
    }

    // url encode body
    if (appProps.server.middleware.urlEncodedBodyParser.enabled) {
      const urlEncodedMiddleware = urlencoded(appProps.server.middleware.urlEncodedBodyParser)
      middlewares.push(
        this.toNodeMiddleware(appProps.server.middleware.urlEncodedBodyParser, (req, res, next) => urlEncodedMiddleware(req, res, next)),
      )
    }
    ...
    if (appProps.server.middleware.multipart.enabled) {
      ...
      middlewares.push(this.toNodeMiddleware(appProps.server.middleware.multipart, (req, res, next) => multipartMiddleware(req, res, next)))
    }
