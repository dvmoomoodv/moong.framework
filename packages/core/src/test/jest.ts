...
import { FRAMEWORK_MODULE_DEFINITION } from '@ustra/core/src/config/framework/modules'
...
FRAMEWORK_MODULE_DEFINITION.forEach(def => {
  ...
  frameworkModuleNameMapper[`${def.alias}/(.*)`] = path.resolve(__dirname, `${FRAMEWORK_BASE_DIR}/${subModuleName}/src/$1`)
  frameworkModuleNameMapper[`^${def.alias}$`] = path.resolve(__dirname, `${FRAMEWORK_BASE_DIR}/${subModuleName}/src`)
})
