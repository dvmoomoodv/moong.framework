...
const devDependencies = frameworkSupport.getAllDevDependencies(baseDir, frameworkDir)
  ...
  .prompt([
    {
      ...
      choices: templates.filter(t => !!t.templateName).map(t => t.templateName),
      loop: true,
    },
  ])
  .then(v => {
    const templateInfo = templates.find(t => t.templateName === v.migrate)
    const sharedPath = path.join(frameworkSupport.getModulePathFromFrameworkPath(frameworkDir, '@ustra/core'), '__templates__', 'shared')

    import('cpx2').then(cpx => {
      const copyAll = (path, update) => {
        fileUtils.getHiddenFiles(path).forEach(file => {
          cpx.copySync(path + '/' + file.name + (file.isDirectory() ? '/*' : ''), baseDir + (file.isDirectory() ? '/' + file.name : ''), {
            update,
