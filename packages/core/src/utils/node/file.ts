...
export class File {
  ...
  getDirectoryFiles(directoryPath: string, onlyDirectory = false): fs.Dirent[] {
    return fs.readdirSync(directoryPath, { withFileTypes: true }).filter(d => !onlyDirectory || d.isDirectory())
  }
  ...
  getHiddenFiles(directoryPath: string): fs.Dirent[] {
    return fs.readdirSync(directoryPath, { withFileTypes: true, encoding: 'utf-8' }).filter(d => d.name.startsWith('.'))
  }
  ...
  fromDir(startPath: string, filter: string, recursive = true): Array<string> {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (stat.isDirectory()) {
        if (recursive) {
          filteredFiles = filteredFiles.concat(...this.fromDir(filename, filter))
        }
   ...
   */
  template(tempaltePath: string, targetPath: string, overwrite: boolean = true, data: object = {}) {
