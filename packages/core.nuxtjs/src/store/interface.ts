...
export const useUstraInterfaceStore = defineInitStore(
  ...
  () => {
     ...
     * 인터페이스 정의 목록
     */
    const interfaceDefinitions = useSessionStorage<
      Map<string, Partial<interfaceModels.InterfaceInfo>>
    >("ustra:interface", new Map(), {
  ...
  async (store, { $ustra, appProps }) => {
    const interfacesMap: Map<
      string,
      Partial<interfaceModels.InterfaceInfo>
    > = new Map();

    // add env definition
    if (appProps.interfaces.definitions) {
      appProps.interfaces.definitions.forEach((i) => {
        const interfaceInfo = $ustra.utils.core.deepMergeWithNull(
          {},
          interfaceModels.DEFAULT_INTERFACE_INFO,
          i
        );
    ...
    if (!store.loadedInterfaceDefinitions) {
      ...
      if (initialDataUrl) {
        // 초기 데이터 조회
        const interfacesResult = await $ustra.api.call<
          apiModels.ApiResponse<interfaceModels.InterfaceInfo[]>
        >({
          url: initialDataUrl,
          method: "POST",
        ...
        interfacesResult.data.body.forEach((i) =>
          interfacesMap.set(i.id + ":" + (i.version || "1.0"), i)
        );
      }
    }
