...
export const useAuthStore = defineInitStore(
  ...
  () => {
    ...
    const serverStore =
      ref<Awaited<ReturnType<typeof useServerAuthStore>>>(null);
    ...
    useServerAuthStore($ustra?.nuxtApp).then(
      (store) => (serverStore.value = store)
    );
    ...
    const states = {
      ...
      authInfo: computed(() => {
        return authType === "jwt"
          ? jwtStore.value.tokenClaim
          : serverStore.value.authInfo;
      }),
      ...
      authenticated: computed(() => {
        return authType === "jwt"
          ? jwtStore.value.authenticated
          : serverStore.value.authenticated;
      }),
      ...
      existsUser: computed(() => {
        return authType === "jwt"
          ? jwtStore.value.existsUser
          : serverStore.value.existsUser;
      }),
      ...
      roles: useStorage<Set<string>>(
        ...
        new Set<string>([]),
        storageType === "local"
          ? core.global.localStorage
          : core.global.sessionStorage,
        {
