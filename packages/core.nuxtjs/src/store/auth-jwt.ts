...
import { jwt, web, core } from "#moong/core/utils";
import pathUtils from "#moong/core/utils/path";
import { CookieStorage } from "#moong/core/utils/storage/cookie-storage";
import { sessionStorage } from "#moong/core/utils/storage/session-storage";
...
export const useJwtAuthStore = defineInitStore(
  ...
  () => {
    ...
    const authenticated = computed(() => {
      return (
        !!tokenClaim.value && !!tokenClaim.value.sub && !!tokenClaim.value.cp
      );
    });
          ...
          const currentSyncCookies =
            syncServerAuthCookies === false
              ...
              : !pathUtils.isMatchPatterns(
                  $ustra.nuxtApp._route.path,
                  ...(cookieOptions.excludeSyncServerUrlPatterns || [])
                );
