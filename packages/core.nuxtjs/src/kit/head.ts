...
export const addScript = async (
  srcScriptPath: string,
  destScriptFilePath: string,
  addVerString: boolean = true,
  includeHeadOption = true
) => {
  const nuxt = useNuxt();

  const { resolve, resolvePath } = createResolver(nuxt.options.rootDir);
  const destScriptFileAbsolutePath = join(
  ...
  if (!existsSync(srcScriptAsolutePath)) {
    console.warn(
      `$ustra : Cannot found script source path : ${srcScriptAsolutePath}`
    );
    return;
  }
  ...
  console.info(
    `$ustra : copy script file : ${srcScriptAsolutePath} -> ${destScriptFileAbsolutePath}`
  );
