import { functions, core } from '#ustra/core/utils'
import masking, { MaskingType } from '#ustra/core/utils/masking'
 ...
 * @param option 옵션
 */
export const useOnError = <T extends Function>(fn: T, option?: OnErrorOptions) => {
 ...
 */
export const useCurrentPageMeta = (route: RouteLocationNormalized = undefined, nuxtApp?: NuxtApp): RouteMeta => {
  if (!route) {
    route = useRoute()
  }
 ...
 * @returns
 */
export const useGetObjectAsync = <T>(objectFn: () => T, timeout = 0): Promise<T> => {
  const $ustra = useUstra()
 ...
 * @param args
 */
export const useMessage = <K extends keyof I18nMessage>(code: K, ...args: any[]) => {
  const $ustra = useUstra()
