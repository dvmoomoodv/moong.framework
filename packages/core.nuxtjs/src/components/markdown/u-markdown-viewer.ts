...
import { h, ref, onMounted, onUpdated, PropType, getCurrentInstance, watch, useUstra } from '#ustra/nuxt'
...
export default defineNuxtComponent({
  ...
  setup(props, { slots, emit }) {
    ...
    const renderMarkdownContent = (markdownContent: string, isHtml: boolean) => {
      ...
      if (!isHtml) {
        markdownContent = markdown.getGlobalMarkdownRenderer().render(markdownContent)
      }
