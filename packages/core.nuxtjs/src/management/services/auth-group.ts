...
import {
  ...
  AuthGroupCriteria,
  AuthGroupTreeData,
} from "../models/auth-group";

/**
 * 권한 그룹 서비스
 */
export const useUstraAuthGroupService = defineUstraService(
  ({ $ustra, api }) => {
    /**
     * 권한 그룹 목록 조회
     * @param creteria 조건
    ...
    async function getAuthGroups(criteria: AuthGroupCriteria = {}) {
      ...
      for (const ag of res.data.body) {
        ...
        if (!treeAuthGroup.uprAuthGrpId) {
          ...
          const uprAuthGroup = findAuthGroupById(
            authGroups,
            treeAuthGroup.uprAuthGrpId
          );
    ...
    async function getRequestableGroupsOfUser(sysCd: string, usrId: string) {
      const url = api
        .urlBuilder("/api/system/auth-group/requestable")
        .add("sysCd", sysCd)
        .add("usrId", usrId)
        .build();
    ...
    async function registerUsers(usrId: string, authGrpIds: number[]) {
      const url = api
        .urlBuilder("/api/system/auth-group/register-users")
        .add("usrId", usrId)
        .addBase64("authGrpIds", authGrpIds)
        .build();
