  ...
  () => {
    ...
    const _actions = {
      updateCurrentMenu(currentMenu) {
        ...
        const findParentMenu = (menu: ProgramMenu, menuNames = []) => {
          ...
          const foundMenus = programMenus.filter(
            (m) => menu.uprMnuId === m.mnuId
          );
        ...
        states.currentProgramMenu.value.mnuFullNm = foundMenus
          .map((m) => m.name)
          .join(" > ");
        states.currentProgramMenu.value.parentMenus = foundMenus
          .map((m) => m.menu)
          .slice(0, foundMenus.length - 1);
      },
    };
    ...
    const actions = {
      ...
      async createNavigation() {
        ...
        const menus = $ustra.utils.core
          .deepMerge([], initDataStore.programMenus)
          .filter((menu) => menu.sysCd === initDataStore.currentSystemCode);
        ...
        const highlightDefault =
          managementConfig.ui.favoriteMenu.highlightDefault;
        ...
        const favoriteVisibleAlways =
          managementConfig.ui.favoriteMenu.visibleAlways;
        const hasFavorites =
          favoriteVisibleAlways || (userMenus && userMenus.length > 0);
        // add favorite
        ...
        $ustra.hooks.callHook(
          "management:navigation:created",
          states.navigations.value
        );
      },
      ...
      loadFavoriteNavigation() {
        ...
        const favoriteVisible =
          favoriteVisibleAlways || favoriteMenuItems.length > 0;
      ...
      setCurrentProgramMenuByPath(path: string) {
        ...
        const programMenus = initDataStore.programMenus;
        if (programMenus) {
          const currentMenu = programMenus.filter(
            (m) =>
              m.sysCd === initDataStore.currentSystemCode &&
              $ustra.utils.router.equalPath(m.mnuUrl, path)
          );
      ...
      setCurrentProgramMenu(id: string) {
        ...
        if (programMenus) {
          const currentMenu = programMenus.filter(
            (m) => m.sysCd === initDataStore.currentSystemCode && m.mnuId === id
          );
      ...
      activateCurrentMenu(
        ...
        if (
          $ustra.utils.router.equalPath($route.path, mainPagePath) &&
          !useTabMenu
        ) {
          const activatedNavigations = states.navigations.value.filter(
            (nav) => nav.visible
          );
      ...
      async loadTabMenus() {
        ...
        const displayHomeMenu =
          $ustra.env.appProps.nuxt.management.ui.displayHomeMenu;
        const mainPagePath =
          $ustra.env.appProps.nuxt.management.ui.defaultPage.main.path;
        const navigations = states.navigations.value;
        ...
        if ($route.query.id) {
          const nav = actions.findNavigationByMenuId(
          ...
          if (!nav) {
            await alert(
              "선택 메뉴는 존재하지 않습니다. 관리자에게 문의하시기 바랍니다."
            );
            window.close();
            return;
          } else {
        ...
        if (displayHomeMenu && mainPagePath && navigations.length > 0) {
          const mainComponent =
            $ustra.utils.router.findComponentByPath(mainPagePath);
      ...
      async loadUserMenus() {
        try {
          const useFavoriteMenu =
            $ustra.env.appProps.nuxt.management.ui.favoriteMenu.enabled;
          ...
          const userMenus = await userMenuService.getUserMenus({
            usrId: $ustra.management.auth.user.sub,
          });
      ...
      findNavigationByMenuId(mnuId: string, checkVisible = false) {
        const navs = $ustra.utils.model.flatReclusiveArray(
          states.navigations.value,
          "items",
          false
        );
