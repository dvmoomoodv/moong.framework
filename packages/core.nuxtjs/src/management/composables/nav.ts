import type { ProgramMenu } from '#ustra/nuxt/management/store/models/navigation'
import { getCurrentInstance, onMounted, markRaw } from '#ustra/nuxt'
 ...
 * @param callback 콜백
 */
export const onUstraNavigationChanged = <T = any>(callback: (menu: ProgramMenu, data: T) => void) => {
  ...
  onMounted(async () => {
    ...
    if (navStore.currentProgramMenu) {
      ...
      navStore.currentProgramMenu.componentCallback(navStore.currentProgramMenu)
    }
  })
}
