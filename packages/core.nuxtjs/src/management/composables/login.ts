...
import { reactive, computed, onBeforeMount, nextTick, useDeepMerge } from '#ustra/nuxt'
import { LoginResult } from '#ustra/nuxt/management/services/common/login'
...
export const useUstraManagementLoginPage = (useRememberId?: boolean) => {
  ...
  async function doLogin(options: {
     ...
     */
    handleLoginResult?: (result: apiModels.ApiResponse<LoginResult>) => Promise<boolean> | boolean
     ...
     */
    onRequirePasswordChange?: (optional: boolean, loginResult: LoginResult) => boolean | Promise<boolean>
     ...
     */
    onRequireApproval?: (type: string, name: string, loginResult: LoginResult) => void | Promise<void>
    ...
    const result = await $ustra.management.auth.login(options.id, options.password, options.completion)
    ...
    if (response.action === 'SUCCESS_RECOMMEND_CHANGE_PASSWORD') {
      ...
      if (options.onRequirePasswordChange) {
        const passwordChangeResult = await options.onRequirePasswordChange(true, result.body)
    ...
    if (response.action === 'FAIL_REQUIRE_APPROVAL') {
      ...
      if (options.onRequireApproval) {
        await options.onRequireApproval(response.approvalType, response.approvalName, result.body)
        return result.body
      }
    }
