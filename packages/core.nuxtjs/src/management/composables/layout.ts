...
import { computed, ref, onMounted, markRaw, nextTick } from "#moong/nuxt";
import { useUstra } from "#moong/nuxt/composables";
import { useUstraManagementUser } from "#moong/nuxt/management";
...
export const useUstraManagementLayoutUtils = () => {
  ...
  const openMenu = async (nav: Navigation, data?: any) => {
    ...
    // tab 메뉴 사용
    // find exists tab
    const existsIndex = openedTabNavigations.value.findIndex(
      (n) => n.id === nav.id
    );
    ...
    if (targetMenu) {
      const accessable = await $ustra.management.auth.accessableMenuIpAddress(
        targetMenu,
        $ustra.management.auth.user.accessInfo.userIp
      );
      if (accessable === false) {
        return;
      }
    ...
    $ustra.management.store.navigation.selectedTabIndex =
      openedTabNavigations.value.length - 1;
  };
  ...
  const closeTabMenuByIndex = (index: number) => {
    ...
    if ($ustra.management.store.navigation.selectedTabIndex === index) {
      const nextIndex =
        openedTabNavigations.value.length - 1 > index ? index : index - 1;
 ...
 * @returns
 */
export const useUstraLayoutManagementSideMenu = (
