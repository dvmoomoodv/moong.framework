...
export const modules = async (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  if (options.nuxt.vuetify.enabled) {
    await installModuleIfNotExists("@moong/nuxt-vuetify", options);
  }

  // wijmo
  if (options.nuxt.wijmo.enabled) {
    await installModuleIfNotExists("@moong/nuxt-wijmo", options);
  }

  // ckeditor
  if (options.nuxt.ckeditor5.enabled) {
    await installModuleIfNotExists("@moong/nuxt-ckeditor5", options);
  }
