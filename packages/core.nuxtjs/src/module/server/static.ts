...
import { StaticServeOptions } from '#ustra/core/server/http/middlewares//static-serve'

import { addTemplate, addServerHandler, resolvePath } from '@nuxt/kit'
import { Nuxt } from '@nuxt/schema'

export const addStaticMiddleware = async (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  for (const staticOption of staticOptions) {
    ...
    staticOption.serveDir = await resolvePath(staticOption.serveDir, { cwd: nuxt.options.srcDir })
  }
  ...
  // when generate copy
    ...
    nuxt.hook('close', nuxt => {
      for (const staticOption of staticOptions) {
        ...
        const destDirPath = resolve(nuxt.options.srcDir, './dist', '.' + staticOption.path)
        logger.info(`$ustra copy static resources... ${staticOption.serveDir} => ${destDirPath}`)
