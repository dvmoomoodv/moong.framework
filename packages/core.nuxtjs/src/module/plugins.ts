...
import { useUstra } from "#moong/nuxt/kit";
...
export const plugins = (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  // add css
  nuxt.options.css.push("@moong/nuxt/src/assets/styles/fonts.scss");
...
import plugin, { addSubSets } from '#moong/nuxt/plugins/ustra'
  ...
  addTemplate({
    ...
    getContents: () => {
      const EXTENSION_RE = new RegExp(
        `(?<=\\w)(${nuxt.options.extensions.map((e) => escapeRE(e)).join("|")})$`,
        "g"
      );
      const tsImports = options.nuxt._build.pluginProps.map((p) => {
        return {
          propName: p.propName,
          modulePath: (isAbsolute(p.modulePath)
            ? relative(join(nuxt.options.buildDir, "types"), p.modulePath)
            : p.modulePath
...
type UstraPluginInjections = { \n  ${tsImports
        .map(
          (p) =>
            `${p.propName}: typeof ${genDynamicImport(p.modulePath, { wrapper: false })}.default`
        )
        .join(" \n  ")}
}

declare module '@moong/nuxt/src/plugins/ustra' {
interface Ustra extends UstraPluginInjections {}
}

declare module '#moong/nuxt/plugins/ustra' {
interface Ustra extends UstraPluginInjections {}
}
...
export { }
  ...
  nuxt.hook("prepare:types", ({ references }) => {
    references.push({
      path: resolve(nuxt.options.buildDir, "types/ustra.d.ts"),
    });
  });

  const findIndexUstraPlugin = (plugins: (NuxtPlugin | string)[]) => {
    return plugins.findIndex(
      (p) => p["filename"] && p["filename"] === "ustra/ustra.ts"
    );
  };
                ...
                : plugin.filename === "ustra/ustra-wijmo.ts"
                  ...
                  : plugin.filename === "ustra/ustra-dx.ts"
                    ...
                    : plugin.filename === "ustra/ustra-ckeditor5.ts"
                      ...
                      : plugin.src &&
                          plugin.src.includes(
                            "@nuxtjs/i18n/dist/runtime/plugins/i18n"
                          )
                        ? 110
                        : plugin.src &&
                            plugin.src.includes("vuetify-nuxt-module")
                          ? 111
                          : // after plugins
  ...
  addTemplate({
    ...
    getContents: () => {
      ...
      for (const propPath of targetProps) {
        ...
        if (
          value &&
          (typeof value === "function" || value.constructor === RegExp)
        ) {
