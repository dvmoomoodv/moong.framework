...
import { createSymlink } from '#ustra/nuxt/kit'
import { logger } from '#ustra/nuxt/utils/logger'
import { module } from '#ustra/core/utils/node'
...
export default defineNuxtModule<NuxtAppProps>({
  ...
  async setup(resolvedOptions, nuxt) {
    ...
    const bannerPath = join(frameworkSupport.getModulePathFromFrameworkPath(frameworkDir, '@ustra/core'), 'src', 'resource', 'banner.txt')
