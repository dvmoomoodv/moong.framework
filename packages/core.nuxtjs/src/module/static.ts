...
async function generateAppProps(options: NuxtAppProps, nuxt: Nuxt) {
  ...
  for (const profile of options.nuxt.build.generation.generateProfiles) {
    // config
    let profileConfig: NuxtAppProps =
      (await config.getProfileNuxtAppProps(nuxt.options.rootDir, profile)) ||
      options;
    ...
    const publicConfig = {
      ...
      [propertyKey]: { ...config.getClientOptions(profileConfig) },
    };
...
async function setSpaLoadingTemplate(options: NuxtAppProps, nuxt: Nuxt) {
  ...
  if (core.isEmpty(nuxt.options.spaLoadingTemplate)) {
    ...
    nuxt.options.spaLoadingTemplate = resolve(
      __dirname,
      "../assets/loading/ustra-spa-loading-template.html"
    );
