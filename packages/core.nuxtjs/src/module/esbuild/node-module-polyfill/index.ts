...
export function NodeModulesPolyfillPlugin(options: NodePolyfillsOptions = {}): Plugin {
  ...
  return {
    ...
    setup: function setup({ onLoad, onResolve, initialOptions }) {
      ...
      async function loader(args: esbuild.OnLoadArgs): Promise<esbuild.OnLoadResult> {
        try {
