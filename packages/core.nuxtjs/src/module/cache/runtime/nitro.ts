...
// @ts-ignore
import keyFunction from "#moong-cache-ssr-key-function";

import { isUrlCacheable } from "./utils";

const customKey = keyFunction
  ? keyFunction
  : options.key
    ? eval(options.key)
    : null;
...
export default async function (nitroApp) {
  ...
  nitroApp.hooks.hook("render:response", async (response, { event }) => {
    const isCacheable = isUrlCacheable(
      event.node.req,
      event.node.res,
      options.pages
    );
    if (isCacheable && response.statusCode === 200) {
      const key = customKey
        ? await customKey(event.node.req.url, event.node.req.headers, event)
        : event.req.url;
