import { getFrameowrkModuleDirPath, resolveFrameworkModulePath } from '#ustra/nuxt/kit'
import { Nuxt } from '@nuxt/schema'
import { logger } from '#ustra/nuxt/utils/logger'
import { encoding, core } from '#ustra/core/utils'
...
const normalizeOptions = (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  nuxt.hook('modules:done', () => {
    ...
    const cryptedKey = crypto.encryptAes256(options.nuxt.env.secret, configureSecret)
    options.nuxt.env.secret = encoding.encodeBase62(configureSecret + '\0' + cryptedKey)

    const publicConfig = { ...(nuxtOptions.runtimeConfig.public || {}), ustra: { ...config.getClientOptions(options) } }

    // @ts-ignore
    nuxtOptions.runtimeConfig = { ...nuxtOptions.runtimeConfig, ustra: { ...options }, public: publicConfig }
  })
...
const configNuxtOptions = (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  // add framework alias
  FRAMEWORK_MODULE_DEFINITION.filter(def => def.requiredImport && def.alias).forEach(def => {
    typeDefinitions.push(...def.typeDefinitions)

    if (nuxt.options.alias[def.alias]) {
      logger.info('$ustra path alias - ', '`' + def.alias + '`', nuxt.options.alias[def.alias])
      return
    }
  ...
  nuxt.hook('imports:dirs', dirs => {
    dirs.push(resolveFrameworkModulePath('#ustra/nuxt/composables'))
    dirs.push(resolveFrameworkModulePath('#ustra/nuxt/composables/components'))
  })
  ...
  nuxt.options.typescript.tsConfig.compilerOptions.types = nuxt.options.typescript.tsConfig.compilerOptions.types || []
  nuxt.options.typescript.tsConfig.compilerOptions.types.push(...typeDefinitions)

  nuxt.options.typescript.shim = false
  nuxt.options.typescript.typeCheck = core.defaults(nuxt.options.typescript.typeCheck, true)
...
export const options = (options: NuxtAppProps, nuxt: Nuxt, frameworkDirPath: string) => {
