...
export class UstraError {
  constructor(private app: NuxtApp) {
    this.app.hook('vue:error', err => {
      ...
      // @ts-ignore
      if (this.app.$ustra.env.appProps.nuxt.error.handlerType === 'redirect' || err?.fatal === true) {
        showError({ message: err?.['message'], fatal: true, statusCode: this.app.$ustra.env.appProps.nuxt.error.statusCode })
      }
    })
