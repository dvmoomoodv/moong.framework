...
import { web, random } from "#moong/core/utils";
import { base64Converter } from "#moong/core/utils/converter/base64-converter";
import NativeAppStorage from "#moong/nuxt/utils/storage/native-app-storage";
...
export class UstraMobile {
  ...
  get isIosNativeRequest() {
    ...
    if (this.mobileProp.hybrid.nativeAgent.ios) {
      return web
        .getUserAgent(this.nuxtApp.ssrContext?.event?.node?.req)
        .includes(this.mobileProp.hybrid.nativeAgent.ios);
    }
  ...
  get isAndroidNativeRequest() {
    ...
    if (this.mobileProp.hybrid.nativeAgent.android) {
      return web
        .getUserAgent(this.nuxtApp.ssrContext?.event?.node?.req)
        .includes(this.mobileProp.hybrid.nativeAgent.android);
    }
...
export class MobileBridge {
  ...
  constructor(
    ...
    const toastFunctionName =
      this.mobileProp.hybrid.bridge.staticBridgeNames.toast;
    ...
    const storageFunctionName =
      this.mobileProp.hybrid.bridge.staticBridgeNames.storage;
  ...
  staticFunctions = {
    ...
    notifyLoaded: () => {
      const functionName =
        this.mobileProp.hybrid.bridge.staticBridgeNames.notifyLoaded;
      if (!functionName) {
        return;
      }
    ...
    currentTime: () => {
      const functionName =
        this.mobileProp.hybrid.bridge.staticBridgeNames.currentTime;
          ...
          this.callNative({
            ...
            callback: (result) => {
              if (result.timeout || result.resCode !== "0000") {
                ...
                resolve(
                  this.nuxtApp.$ustra.utils.date.parse(
  ...
  private async createToken() {
    ...
    const currentTimeStamp = this.nuxtApp.$ustra.utils.date.format(
      currentDate,
      "yyyyMMddHHmmss"
    );
  ...
  private async validateToken(token: string) {
    ...
    try {
      ...
      const tokenIssueDate = this.nuxtApp.$ustra.utils.date.parse(
        issueTimeStamp,
        "yyyyMMddHHmmss"
      );
      ...
      // 발행 기간 10초가 넘을 경우
      if (
        this.nuxtApp.$ustra.utils.date.getMilliDuration(
          currentDate,
          tokenIssueDate
        ) > 10000
      ) {
  ...
  private callTimeCallback(callbackFnId: string) {
    if (
      ...
      !window[callbackFnId].callback
    ) {
      return;
    }

    logger.error(
      "mobile app interface timeout : " + window[callbackFnId].callInterfaceId
    );
    window[callbackFnId].called = true;
  ...
  async callNative<T = any>(option: MobileBridgeCallOption<T>) {
    ...
    const interfaceInfo = this.nuxtApp.$ustra.interfaces.getOutboundInterface(
      option.id,
      "MOBILE_BRIDGE"
    );
    ...
    const callbackFnId: string =
      option.callbackName || callInterfaceId + "_callback";
    ...
    if (option.callback) {
      ...
      window[callbackFnId] = this.nuxtApp.$ustra.utils.functions.wrap(
        option.callback,
        (next, ...args) => {
          if (window[callbackFnId]?.timer) {
            clearTimeout(window[callbackFnId].timer);
   ...
   */
  addNativeListener<T = any>(
    ifId: string,
    listener: (
      param: MobileBridgeResult<T>
  ...
  private registerNativeInboundFunctions() {
    const interfaces = this.nuxtApp.$ustra.interfaces
      .getInterfaces()
      .filter((i) => i.direction === "INBOUND" && i.type === "MOBILE_BRIDGE");
      ...
      window[callInterfaceId] = async (param) => {
        ...
        if (!param) {
          ...
          if (!interfaceInfo) {
            ...
            interfaceInfo.allowedAuthenticated &&
            !this.nuxtApp.$ustra.auth.isAuthenticated
          ) {
            result.resCode = "0401";
        ...
        if (param.callbackFnName) {
          // call back 처리
          this.nuxtApp.$ustra.utils.mobile.callNative(
        ...
        logger.debug(
          "receive callback native interface : ",
          callInterfaceId,
          result
        );
        return result;
