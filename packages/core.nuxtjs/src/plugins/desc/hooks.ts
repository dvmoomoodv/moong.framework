...
export interface UstraNuxtRuntimeHooks {
   ...
   * 인증 정보 검증
   */
  'auth:validate': (arg: { $ustra: Ustra; to: RouteLocationNormalized; from: RouteLocationNormalized }) => any
   ...
   */
  'auth:inactivated': (authInfo: any, type: 'expired' | 'duplicated' | 'released', lastCallUrl: string) => any
   ...
   */
  'management:initial-data:loaded': (info: UstraManagementInitialDataLoaded) => void | Promise<void>
   ...
   */
  'management:navigation:updated': (navigation: Navigation, currentRoute: RouteLocationNormalized) => void | Promise<void>

  /**
   * management navigation 생성
   */
  'management:navigation:created': (navigations: Navigation[]) => void | Promise<void>

  /**
   * management 인증 처리 완료 시
   */
  'management:auth:authenticated': (result: LoginResult) => void | Promise<void>
...
const __onceHooks = ['management:initial-data:loaded', 'management:initial-data:error', 'env:loaded']
...
export class UstraHooks extends Hookable<UstraNuxtRuntimeHooks> {
  ...
  callHook<NameT extends HookKeys<UstraNuxtRuntimeHooks>>(
    name: NameT,
    ...args: Parameters<UstraNuxtRuntimeHooks[NameT] extends HookCallback ? UstraNuxtRuntimeHooks[NameT] : never>
  ): Promise<any> {
  ...
  hook<NameT extends HookKeys<UstraNuxtRuntimeHooks>>(
    name: NameT,
    fn: UstraNuxtRuntimeHooks[NameT] extends HookCallback ? UstraNuxtRuntimeHooks[NameT] : never,
  ): () => void {
