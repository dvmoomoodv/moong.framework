...
import { path, jwt, array, url } from "#moong/core/utils";
import { useLogger } from "#moong/nuxt/utils/logger";
import { useUstra, ref } from "#moong/nuxt";
import { useCurrentPageMeta } from "#moong/nuxt/composables/utils";
import { useAuthStore } from "#moong/nuxt/store/auth";
import { useJwtAuthStore } from "#moong/nuxt/store/auth-jwt";
import { useServerAuthStore } from "#moong/nuxt/store/auth-server";
...
export class UstraAuth {
  ...
  async init() {
    ...
    this.timer = new UstraAuthTimer(this.$ustra, {
      onExpired: () => {
        this.inactivate(
          "expired",
          null,
          "authentication timer is expired",
          null,
          false
        );
      },
    ...
    axiosInstance.interceptors.request.use((config) => {
      ...
      if (this.authProps.type === "jwt") {
        // has auth token
        if (
          config.headers[this.authProps.jwt.refreshTokenKey] ||
          config.headers[this.authProps.jwt.accessTokenKey]
        ) {
          return config;
        }
    ...
    this.$ustra.api.getSseInstance().addRequestInterceptor((option) => {
      ...
      if (this.authProps.type === "jwt") {
        // has auth token
        if (
          option.headers[this.authProps.jwt.refreshTokenKey] ||
          option.headers[this.authProps.jwt.accessTokenKey]
        ) {
          return option;
        }
        const headers = this.getHeaders();
    ...
    const checkResponse = async (res) => {
      ...
      if (
        this.isExcludeUrlPattern(res?.config.url) ||
        !!res.config["excludeAuthValidation"]
      ) {
        return res;
      }
      if (this.authProps.type === "jwt") {
        ...
        if (res.headers) {
          ...
          const accessToken =
            res.headers[toLower(this.authProps.jwt.accessTokenKey)] || null;
          const refreshToken =
            res.headers[toLower(this.authProps.jwt.refreshTokenKey)] || null;
  ...
  async validateAuthentication(
    ...
    if (!array.isEmpty($ustra.env.appProps.auth.excludeUrlPatterns)) {
      if (
        path.isMatchPatterns(
          to.path,
          ...$ustra.env.appProps.auth.excludeUrlPatterns
        )
      ) {
        return;
      }
    ...
    if (
      process.server &&
      $ustra.env.appProps.auth.type === "jwt" &&
      !$ustra.env.appProps.auth.jwt.useCookie
    ) {
      return;
    }
    ...
    if (meta.auth?.validateAuthentication) {
      validationResult = await meta.auth.validateAuthentication(
        $ustra,
        to,
        from
      );
    ...
    if ($ustra.utils.core.isEmpty(validationResult)) {
      validationResult = await $ustra.hooks.callHook("auth:validate", {
        $ustra,
        to,
        from,
      });
    ...
    if ($ustra.utils.core.isEmpty(validationResult)) {
      ...
      if (validationResult !== false) {
        if (meta.auth.roles && meta.auth.roles.length > 0) {
          if (
            !meta.auth.roles.some((role) => $ustra.auth.store.roles.has(role))
          ) {
            validationResult = false;
          }
        }
    ...
    if (validationResult === false) {
      if (isMiddleware) {
        const result = await this.inactivate(
          "validate",
          { to, from },
          "fail validation authentication from auth middleware",
          to.path,
          isMiddleware
        );
        return result;
      } else {
        return await this.inactivate(
          "validate",
          { to, from },
          "fail validation authentication from client plugin",
          to.path,
          isMiddleware
        );
      }
    ...
    return callWithNuxt(this.$ustra.nuxtApp, () => {
      ...
      if (loginPath !== to) {
        if (this.authProps.redirectReturnPageParamName) {
          const returnPageParamValue = url
            .getUrlSearchParam(currentPath)
            .get(this.authProps.redirectReturnPageParamName);
            ...
            return navigateTo(
              {
                path: loginPath,
                query: {
                  [this.authProps.redirectReturnPageParamName]: currentPath,
                },
              },
              { replace: true }
            );
  ...
  public getHeaders() {
    ...
    if (this.authProps.type === "jwt") {
      ...
      if (this._jwtStore.refreshToken) {
        headers[this.authProps.jwt.refreshTokenKey] =
          this._jwtStore.refreshToken;
      }
    }
    return headers;
  }
  ...
  async inactivate(
    ...
    // 검증 실패
    if (type === "validate") {
      const invalidatedResult = await this.$ustra.hooks.callHook(
        "auth:invalidated",
        authInfo,
        to,
        isMiddleware
...
class UstraAuthTimer {
  ...
  private timer = useTimeoutFn(
    () => {
      ...
      this.remainSec.value = Math.ceil(
        $ustra.utils.date.getMilliDuration(new Date(), this.authNoExpireTime) /
          1000
      );
  ...
  private startTimer() {
    ...
    this.authNoExpireTime = $ustra.utils.date.addSeconds(
      new Date(),
...
class UstraAuthDuplicationChecker {
  ...
  private start() {
    if (
      this.authProps.duplication.type === "none" ||
      this.authProps.type !== "jwt"
    ) {
      return;
    }
  }
