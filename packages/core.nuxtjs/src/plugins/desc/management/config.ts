import { Ustra, markRaw } from '#ustra/nuxt'
...
export class UstraManagementConfig {
  ...
  async getConfigMenus(): Promise<Partial<ConfigMenu>[]> {
    ...
    defaultMenus.forEach(menu => {
      if (menu.id === 'password') {
        // 비밀번호 업데이트 가능 여부 적용
        menu.visible = this.$ustra.management.serverAppProps.canUpdateUserPassword
      }
    })

    const configMenus = ((await this.$ustra.hooks.callHook('management:config:menu:load', defaultMenus)) || defaultMenus) as Partial<ConfigMenu>[]
