...
import {
  ...
  LoginResult,
} from "../../../management/services/common/login";
...
export class UstraManagmentAuth {
  ...
  private init() {
    ...
    this.$ustra.hooks.hook("management:auth:authenticated", async (result) => {
      // add roles
      this.$ustra.auth.store.roles = new Set<string>(
        result.authorities.map((authority) => authority.clientAuthority)
      );
  ...
  get useAuthApproval() {
    return (
      $ustra.management.store.initData.serverAppProps.authApprovalType !==
      "NONE"
    );
  }
  ...
  get maximumLoginFailNumbers() {
    return $ustra.management.store.initData.serverAppProps
   ...
   * @returns
   */
  async nomalizeLoginResult(
    result: apiModels.ApiResponse<LoginResult>,
    callHook: boolean = true
  ) {
    result.body = result.body || {};
    result.body.actionResponse = {};
    result.body.actionResponse.nextPath = this.getNextPath(
      result.body.userMenus
    );
    ...
    if (result.body.resCd === "0000") {
      if (callHook) {
        await this.$ustra.hooks.callHook(
      ...
      if (
        this.passwordChangeDays &&
        this.passwordChangeDays < result.body.pwdUnchangedDays
      ) {
        result.body.actionResponse = {
          ...result.body.actionResponse,
    ...
    if (result.body.hasUser) {
      if (
        this.maximumLoginFailNumbers > 0 &&
        result.body.loginFailCnt >= this.maximumLoginFailNumbers
      ) {
        if (!this.useAuthApproval) {
          result.body.actionResponse = {
            ...result.body.actionResponse,
  ...
  hasFunctionAuthority(functionId: string) {
    const currentMenu =
      this.$ustra.management.store.navigation.currentProgramMenu;
  ...
  async accessableMenuIpAddress(currentMenu: Menu, ipAddress: string) {
    if (customIpAddressChecker) {
      ...
      const customIpAddressCheckerResult = await customIpAddressChecker(
        currentMenu,
        ipAddress
      );

      if (
        customIpAddressCheckerResult === true ||
        customIpAddressCheckerResult === false
      ) {
    ...
    if (currentMenu.ipLmtYn === "Y" && currentMenu.ipList) {
      ...
      if (!currentMenu.ipList.split(",").includes(ipAddress)) {
        ...
        if (this.$ustra.env.appProps.nuxt.management.ui.tabMenu.enabled) {
          ...
          await alert(
            this.$ustra.message.getMessage(
              "ustra.management.cannotAccessbleMenuIpAddress"
            )
          );
        } else {
          await this.$ustra.auth.inactivate(
            "validate",
            null,
            "user connected from not allowed ip address."
          );
        }
