...
export class UstraManagement {
  private _initDataStore: Awaited<ReturnType<typeof useUstraInitDataStore>> =
    null;
  private _navigationStore: Awaited<
    ReturnType<typeof useUstraNavigationStore>
  > = null;
        ...
        if (meta.auth?.required && currentMenu) {
          ...
          if (!this.auth.hasMenuAuthority(currentMenu.mnuId)) {
            await this.$ustra.auth.inactivate(
              "validate",
              null,
              "user has not authority for current menu."
            );
            return;
          }
          ...
          // check ip address
          if (
            (await this.auth.accessableMenuIpAddress(
              currentMenu,
              this.auth.user.accessInfo.userIp
            )) === false
          ) {
            return;
          }
          ...
          if (
            ...
            throw createError({
              statusCode: 404,
              message: this.$ustra.message.getMessage(
                "ustra.error.notFoundUrl"
              ),
