...
import { interfaceModels, apiModels } from "#moong/core/data";
...
export class UstraInterfaces {
  ...
  getOutboundInterface = (id: string, type: string = null) => {
    const interfaceDefinitions =
      this.nuxtApp.$ustra.definedStore.interfaces.interfaceDefinitions;
    ...
    return interfaces.find(
      ([key, i]) =>
        i.id === id && i.direction === "OUTBOUND" && (!type || i.type === type)
    )?.[1];
  };
  ...
  getInboundInterface = (id: string, type: string = null) => {
    ...
    return interfaces.find(
      ([key, i]) =>
        i.id === id && i.direction === "INBOUND" && (!type || i.type === type)
    );
  };
  ...
  getInterfaces = () => {
    const interfaceDefinitions =
      this.nuxtApp.$ustra.definedStore.interfaces.interfaceDefinitions;
    const interfaces = Array.from(interfaceDefinitions.entries()).map(
      ([key, i]) => i
    );
   ...
   * @param ifsInfo
   */
  public async addRequestLogging(
    logginInfo: interfaceModels.InterfaceRequestLogging
  ) {
    ...
    try {
      if (logginInfo.requestContent) {
        logginInfo.requestContent =
          typeof logginInfo.requestContent === "string"
            ? logginInfo.requestContent
            : JSON.stringify(logginInfo.requestContent);
      }

      const result = await this.nuxtApp.$ustra.api.call<
        apiModels.ApiResponse<number>
      >({
        url: apiUrl,
        data: logginInfo,
      });
   ...
   * 응답 로깅 정보 추가
   */
  public addResponseLogging(
    loggingData: interfaceModels.InterfaceResponseLogging
  ) {
