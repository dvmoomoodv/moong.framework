<template>
  <ul class="group_btn_list" :class="{ 'no-border': !props.border }" :style="styles">
    <li v-for="(item, index) in props.itemsSource" :key="index" :style="{ textAlign: props.itemAlign, width: props.stack ? 'auto' : null }">
      <input
        type="checkbox"
        :id="name + index"
        :name="name"
      ...
      />
      <label :for="name + index" @click="e => onItemClick(e, item, index)">{{ item.text }}</label>
    </li>
  </ul>
</template>
<script lang="ts" setup>
import { defineProps, withDefaults, defineEmits, computed, watch, reactive, nextTick } from '#ustra/nuxt'
...
async function onItemClick(e, item, index) {
  ...
  if (item.allSelectedItem) {
    ...
    for (let i = 0; i < values.value.length; i++) {
      ...
      setTimeout(() => (values.value[i] = v ? trueValue(props.itemsSource[i].trueValue) : falseValue(props.itemsSource[i].falseValue)), 1)
    }
    e.value = v
  } else {
    if (!isMultiple.value) {
      const v = values.value[index] === trueValue(item.trueValue) ? falseValue(item.falseValue) : trueValue(item.trueValue)
      e.value = v
    } else {
      const v = $ustra.utils.core.deepMerge([], values.value)
      v[index] = v[index] === trueValue(item.trueValue) ? falseValue(item.falseValue) : trueValue(item.trueValue)
      e.value = v
    }
  }
