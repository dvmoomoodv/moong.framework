...
import { computed, useAttrs, defineProps, nextTick, shallowRef, withDefaults } from '#ustra/nuxt'
import { validation } from '#ustra/core/utils'
import { useVModel } from '@vueuse/core'

const textControl = shallowRef<InstanceType<typeof VTextField>>()
const inputEl = computed(() => textControl.value.$el.querySelector('input') as HTMLInputElement)
...
function formatValue(e) {
  if (
    ...
    nextTick(() => {
      ...
      } else if (value.length >= 3 && !value.startsWith('050') && !value.startsWith('15') && !value.startsWith('16')) {
        value = value.substring(0, 3) + '-' + value.substring(3)
      } else if (value.length >= 4 && (value.startsWith('050') || value.startsWith('15') || value.startsWith('16'))) {
      ...
      if (restValue.length === 3 || restValue.length === 4) {
        ...
        value = orgValue + restValue.substring(0, 3) + '-' + restValue.substring(3)
      } else if (restValue.length >= 8) {
        value = orgValue + restValue.substring(0, 4) + '-' + restValue.substring(4)
      }
