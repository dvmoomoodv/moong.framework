  ...
  <UBox direction="col">
    ...
    <UItem :ratio="1">
      <UBox direction="col">
        <WjFlexGrid
          style="height: 500px"
          :initialized="gridAction.histGrid.initialize"
        >
          <WjFlexGridColumn
            binding="batHistId"
            header="배치 이력 아이디"
          ...
          <WjFlexGridColumn
            ...
            header="실행시간"
            width="*"
            :cellTemplate="(ctx) => gridAction.renderUsePeroid(ctx.item)"
          />
          ...
          <WjFlexGridColumn binding="batHistId" header="작업" :width="120">
            <WjFlexGridCellTemplate cellType="Cell" v-slot="cell">
              <div>
                <UButtonBox>
                  <UButton
                    @click="gridAction.printLog(cell.item.batHistId)"
                    small
                    type="primary"
                    text="로그"
                    :width="80"
...
import { useWijmoFlexGrid } from "#moong/nuxt-wijmo/composables/grid";
import {
  BatchHistCriteria,
  BatchHist,
