          ...
          <UButtonBar class="mt-2">
            <UButtonBox right>
              ...
              <VBtn
                ...
                :disabled="ifsKey.disabledRemoveButton.value"
                @click="() => ifsKey.removeRow()"
              ></VBtn>
            </UButtonBox>
          </UButtonBar>
        </UFieldRow>
...
import { FormMode } from "#moong/core/data/models/base";
import IfsKeyForm from "./ifs-key-form.vue";
import { IfsCriteria, Ifs } from "#moong/nuxt/management";
import { useUstraInterfaceService } from "#moong/nuxt/management/services/interface";
...
const saveActions = (() => {
  ...
  function validate() {
    if (inputData.ifTyCd === "SP" && inputData.httpMethCd !== "POST") {
      ...
      (inputData.httpMethCd === "DELETE" ||
        inputData.httpMethCd === "PATCH" ||
        inputData.httpMethCd === "PUT")
    ) {
