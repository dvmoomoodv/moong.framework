      ...
      <UFieldSet>
        <UFieldRow>
          <UField label="시스템" totalWidth="400">
            <UVCodeComboBox
              grpCd="SYS_CD"
              v-model="searchBarActions.selectedSystemCode.value"
            />
          </UField>
        </UFieldRow>
      </UFieldSet>
            ...
            <UItem>
              <UButtonBar>
                <UButtonBox right top>
                  <VBtn
                    text="권한그룹 추가"
                    v-if="selectedAuthGrpId == null"
                    :width="100"
                    @click="() => formActions.init()"
                  />
                  <VBtn
                    text="하위 권한그룹 추가"
                    v-if="selectedAuthGrpId != null"
                    :width="140"
                    @click="() => formActions.init(selectedAuthGrpId)"
                  />
                </UButtonBox>
              </UButtonBar>
            </UItem>
    ...
    return useOnError(
      async function () {
        ...
        items.value = await authGroupService.getAuthGroups({
          sysCd: searchBarActions.selectedSystemCode.value.dtlCd,
        });
...
const searchBarActions = (function () {
  ...
  onMounted(() => {
    watch(selectedSystemCode, () => treeActions.loadAuthGroups(), {
      immediate: true,
    });
  });
