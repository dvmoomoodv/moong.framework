...
import { NuxtAppProps, VuetifySampleMenuOptions } from '#ustra/nuxt/config'
import { path as pathUtils, model, core } from '#ustra/core/utils'
...
function createAdditionalRouter(options: NuxtAppProps, nuxt: Nuxt) {
  ...
  nuxt.options['ustra'].nuxt.vuetify.samples.additionalMenus = options.nuxt.vuetify.samples.additionalMenus
...
export const samples = async (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  let basePath = '#ustra/nuxt-vuetify/samples'
  // copy resource
  if (sampleProps.copyResource.enabled) {
    const targetDir = resolve(__dirname, '../samples')
    const destDir = path.getAbsolutePath(sampleProps.copyResource.targetDirPath + sampleProps.path, nuxt.options.srcDir)
  ...
  nuxt.hook('pages:extend', async pages => {
    pages.push({
      ...
      file: '#ustra/nuxt-vuetify/samples/index.vue',
      children: (
        await createChildrenRouter(options, options.nuxt.management.enabled && options.nuxt.management.ui.componentType === 'vuetify', basePath)
