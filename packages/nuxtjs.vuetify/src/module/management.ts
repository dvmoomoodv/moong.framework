...
function addApp(options: NuxtAppProps, nuxt: Nuxt) {
  ...
  addComponent({
    ...
    filePath: resolve(
      currentDir,
      "../management/apps/ustra-management-app.vue"
    ),
    global: true,
  });
  ...
  addComponent({
    ...
    filePath: resolve(
      currentDir,
      "../management/apps/ustra-management-error.vue"
    ),
    global: true,
  });
}
...
function addLoginPage(options: NuxtAppProps, nuxt: Nuxt) {
  if (
    !options.nuxt.management.ui.defaultPage.login.include ||
  ...
  nuxt.hook("pages:extend", (pages) => {
    const path =
      options.nuxt.management.ui.defaultPage.login.path ||
      options.auth.loginPath;
    ...
    if (loginPage) {
      ...
      logger.info(
        `$ustra add management login page : ${options.auth.loginPath}`
      );
    }
  });
}
...
function addMainPage(options: NuxtAppProps, nuxt: Nuxt) {
  if (
    !options.nuxt.management.ui.defaultPage.main.include ||
    !options.nuxt.management.ui.defaultPage.main.path
  ) {
    return;
  }
...
async function addSystemPage(options: NuxtAppProps, nuxt: Nuxt) {
  ...
  if (systemPageProps.copyResource.enabled) {
    ...
    const destDir = path.getAbsolutePath(
      systemPageProps.copyResource.targetDirPath + "/system",
      nuxt.options.srcDir
    );
    ...
    const commonDestDir = path.getAbsolutePath(
      systemPageProps.copyResource.targetDirPath + "/common",
      nuxt.options.srcDir
    );
  ...
  nuxt.hook("pages:extend", (pages) => {
    ...
    for (const dir of dirs) {
      ...
      if (pages.some((page) => page.path === urlPath)) {
        logger.warn(
          `system page path component already registered. : ${urlPath}`
        );
        return;
      }
      pages.push({
      ...
      logger.info(
        `$ustra add management system page : ${urlPath}, ${componentPath}`
      );
    }
  });
}
...
export const management = async (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  nuxt.hook("imports:dirs", (dirs) => {
    dirs.push(
      resolveFrameworkModulePath("#moong/nuxt-vuetify/management/composables")
    );
  });
