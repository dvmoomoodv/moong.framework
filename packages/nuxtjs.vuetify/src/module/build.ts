...
export const build = async (options: NuxtAppProps, nuxt: Nuxt) => {
  ...
  extendViteConfig(config => {
    ...
    config.optimizeDeps.exclude = [...(config.optimizeDeps.exclude || []), 'vuetify/lib/components/index.mjs']

    if (options.nuxt.vuetify.datepicker.enabled) {
      config.optimizeDeps.exclude = [...(config.optimizeDeps.exclude || []), '@vuepic/vue-datepicker']
    }
  })

  // for ssr build patch
  // @see https://vuetify-nuxt-module.netlify.app/guide/server-side-rendering.html#vuetify-sass-variables
  if (nuxt.options.ssr && options.nuxt.vuetify?.moduleOptions?.styles?.configFile) {
    // @ts-expect-error
    nuxt.options.features ||= {}
    nuxt.options.features.inlineStyles = false

    logger.info('set nuxt.options.features.inlineStyles to false automatically when use SSR and custom sass files.')
  }
