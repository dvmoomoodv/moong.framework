import { defineAsyncComponent } from "#moong/nuxt";
    ...
    $ustra.hooks.hook("management:config:menu:load", (configMenus) => {
      configMenus.forEach((menu) => {
        ...
        if (menu.id === "application") {
          menu.component = defineAsyncComponent(
            () =>
              import(
                "#moong/nuxt-vuetify/management/layouts/config/application.vue"
              )
          );
        }
        ...
        if (menu.id === "password") {
          menu.component = defineAsyncComponent(
            () =>
              import(
                "#moong/nuxt-vuetify/management/layouts/config/password.vue"
              )
          );
        }
      });
