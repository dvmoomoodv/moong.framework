...
import { useUstraManagementApp } from "#moong/nuxt/management/composables";

defineOptions({
  name: "UstraManagementApp",
});

const UDialog = defineAsyncComponent(() =>
  import("#moong/nuxt-vuetify/components").then((c) => c.UDialog)
);
const ULoadingBar = defineAsyncComponent(() =>
  import("#moong/nuxt-vuetify/components").then((c) => c.ULoadingBar)
);
const ConfigPopup = defineAsyncComponent(() =>
  import("#moong/nuxt-vuetify/management/layouts/config-popup.vue").then(
    (c) => c.default
  )
);
const UstraManagementPasswordEditPopup = defineAsyncComponent(() =>
  import(
    "#moong/nuxt-vuetify/management/pages/common/password-edit-popup.vue"
  ).then((c) => c.default)
);
...
function getMainClasses() {
  ...
  if (
    $ustra.env.appProps.nuxt.vuetify.samples.enabled &&
    route.path.startsWith($ustra.env.appProps.nuxt.vuetify.samples.path)
  ) {
...
watch(
  ...
  (path) => {
    ...
    setPageLayout(
      $ustra.management.navigation.getCurrentRouteLayoutName(currentRoute)
    );
  },
  {
    immediate: true,
...
// 팝업
const { showConfigPopup, showPasswordPopup, passwordPopup, events } =
  useUstraManagementApp();
