  ...
  <UBox height="100%" width="100%" direction="row">
    <UItem :ratio="1">
      ...
      <UItem>
        <UButtonBar>
          <UButtonBox>
            <VBtn text="메뉴 추가" v-if="selectedMnuId == null" :width="100" @click="() => init()" />
            <VBtn text="하위 메뉴 추가" v-if="selectedMnuId != null" :width="140" @click="() => init(selectedMnuId)" />
          </UButtonBox>
        </UButtonBar>
      </UItem>
    </UItem>
...
import { useUstraMenuService } from '#ustra/nuxt/management'
...
async function load(){
  ...
  itemIter.forEach(item => {
    const { uprMnuId, mnuId, mnuNm, delYn, dpYn, ipLmtYn, mnuDesc, mnuDvcScopCd, mnuStrNo, mnuStepNo, mnuTyCd, prvViewYn, sysCd, useYn, mnuUrl } = item
    nodes.value[mnuId] = { 
      text: mnuNm,
      delYn: delYn,
      dpYn: dpYn, 
