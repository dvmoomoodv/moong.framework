      ...
      <UFieldSet>
        <UFieldRow :ratio="[1, 1, 1, 1]">
          <UField direction="row" label="프로세스 아이디">
            <VTextField
              type="text"
              v-model="searchAction.searchParam.procId"
              style="width: 230px"
            />
          </UField>
          <UField direction="row" label="요청 아이디">
            <VTextField
              type="text"
              v-model="searchAction.searchParam.reqId"
              style="width: 230px"
            />
          </UField>
          <UField direction="row" label="인터페이스 아이디">
            <VTextField
              type="text"
              v-model="searchAction.searchParam.ifId"
              style="width: 230px"
            />
          </UField>
          <UField direction="row" label="URL">
            <VTextField
              type="text"
              v-model="searchAction.searchParam.url"
              style="width: 230px"
            />
          </UField>
        </UFieldRow>
        <UFieldRow :ratio="[1, 1, 1, 1]">
          <UField direction="row" label="채널 코드">
            <UVCodeComboBox
              grpCd="CHNL_CD"
              v-model="searchAction.searchParam.chnlCd"
              style="width: 230px"
              :displayNullText="'전체'"
            />
          </UField>
          ...
          <UField direction="row" label="응답 코드 값">
            <VTextField
              type="text"
              v-model="searchAction.searchParam.resCdVal"
              style="width: 230px"
            />
          </UField>
        </UFieldRow>
    ...
    <UItem :ratio="1">
      <UBox direction="row" style="gap: 5px">
        ...
        <UItem :ratio="3" v-if="formAction.isFormVisible.value">
          <UFieldSet>
            <UFieldRow>
              <UField label="요청 헤더 내용"
                ><VTextField
                  type="textarea"
                  rows="2"
                  noResize
                  v-model="formAction.inputData.reqHedCont"
                />
              </UField>
            </UFieldRow>
            <UFieldRow>
              <UField label="요청 파라메터 내용"
                ><VTextField
                  type="textarea"
                  rows="2"
                  noResize
                  v-model="formAction.inputData.reqPrmCont"
                />
              </UField>
            </UFieldRow>
            <UFieldRow>
              <UField label="요청 메시지"
                ><VTextField
                  ...
                  rows="2"
                  noResize
                  v-model="formAction.inputData.reqMsgCont"
                />
              </UField>
            </UFieldRow>
            <UFieldRow>
              <UField label="응답 헤더 내용"
                ><VTextField
                  type="textarea"
                  rows="2"
                  noResize
                  v-model="formAction.inputData.resHedCont"
                />
              </UField>
            </UFieldRow>
            <UFieldRow>
              <UField label="응답 메시지"
                ><VTextField
                  ...
                  rows="2"
                  noResize
                  v-model="formAction.inputData.resMsgCont"
                />
              </UField>
            </UFieldRow>
            <UFieldRow>
              <UField label="비고"
                ><VTextField
                  ...
                  rows="2"
                  noResize
                  v-model="formAction.inputData.rmk"
                />
              </UField>
            </UFieldRow>
            <UFieldRow>
...
import { ref, reactive, useOnError } from "#moong/nuxt";
import { IfsCriteria, IfsHist } from "#moong/nuxt/management/models/interfaces";
import { useUstraInterfaceService } from "#moong/nuxt/management";
...
const searchAction = (function () {
  const searchParam: IfsCriteria = reactive({
    ...
    searchSrtDttm: $ustra.utils.formatting.date(
      $ustra.utils.date.addDays(new Date(), -7),
      "yyyyMMddHHmm"
    ),
...
const gridAction = (function () {
  ...
  async function load() {
    ...
    saveData.searchSrtDttm = searchAction.searchParam.searchSrtDttm
      ? searchAction.searchParam.searchSrtDttm.replace(
          /(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})/,
          "$1-$2-$3 $4:$5"
        )
      : null;
    saveData.searchEndDttm = searchAction.searchParam.searchEndDttm
      ? searchAction.searchParam.searchEndDttm.replace(
          /(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})/,
          "$1-$2-$3 $4:$5"
        )
      : null;
