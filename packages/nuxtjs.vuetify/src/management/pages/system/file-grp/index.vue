        ...
        <UItem :ratio="1">
          <UFieldSet>
            ...
            <UFieldRow>
              <UField label="확장자제한">
                <VTextField
                  v-model="formActions.inputData.extenLmt"
                  :isRequired="false"
                />
                <UMessage type="info">
                  미 입력시 모든 유형에 대해 업로드가 가능합니다.<br />
                  확장자 또는 Mime type별로 제한을 설정하는 경우, comma로
                  구분하여 입력해주세요. <br />
            ...
            <UFieldRow>
              <UField required label="파일구조">
                <UVCodeComboBox
                  grpCd="DIR_STRUCT_CD"
                  v-model="formActions.inputData.dirStructCd"
                />
              </UField>
            </UFieldRow>
            ...
            <UFieldRow>
              <UField required label="파일명 저장방식">
                <UVCodeComboBox
                  grpCd="FILE_NM_SV_METH_CD"
                  v-model="formActions.inputData.fileNmSvMethCd"
                />
              </UField>
            </UFieldRow>
            ...
            <UFieldRow>
              <UField label="웹 기본 url">
                <VTextField
                  v-model="formActions.inputData.webDefUrl"
                  :isRequired="false"
                />
              </UField>
            </UFieldRow>
            ...
            <UFieldRow>
              <UField required label="파일 암호화">
                <UVCodeComboBox
                  grpCd="FILE_CRYT_METH_CD"
                  v-model="formActions.inputData.fileCrytMethCd"
                />
              </UField>
            </UFieldRow>
...
import { useUstraFileGroupService } from "#moong/nuxt/management/composables";
import { FileGrp } from "#moong/nuxt/management";
...
const listActions = (() => {
  ...
  const fileGroups = ref<
    Awaited<ReturnType<typeof fileGroupService.getFileGroups>>
  >([]);
  ...
  async function load(fileGrpId: string = null) {
    ...
    if (fileGrpId) {
      await nextTick();
      const index = fileGroups.value.findIndex(
        (c) => c.fileGrpId === fileGrpId
      );
    }
  }
