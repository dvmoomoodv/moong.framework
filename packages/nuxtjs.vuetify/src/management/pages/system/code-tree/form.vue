...
async function loadDetail() {
  if (isEditMode.value) {
    const codeInfo = await codeService.getCode(
      props.selectedCode.grpCd,
      props.selectedCode.dtlCd
    );
...
async function save() {
  return useOnError(
    async () => {
      ...
      saveData.grpCd =
        props.depth === 0 ? saveData.dtlCd : props.selectedCode.grpCd;
      saveData.dtlCd = props.depth === 0 ? "*" : saveData.dtlCd;
      saveData.grpNm =
        props.depth === 0 ? saveData.grpNm : props.selectedCode.grpNm;
      ...
      emits(
        "updated",
        props.depth,
        props.depth === 0 ? saveData.grpCd : saveData.dtlCd
      );
    },
    {
...
async function remove() {
  return useOnError(
    async () => {
      if (props.depth === 0) {
        const codeListCheck = await codeService.getCodesByGroup(
          inputFormData.grpCd
        );
        ...
        const codeListCheck = await codeService.getCodeGroupDepth(
          inputFormData.grpCd,
          inputFormData.dtlCd
        );
