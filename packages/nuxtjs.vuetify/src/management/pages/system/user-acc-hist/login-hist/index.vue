...
import UVDatePeriodBox from "#moong/nuxt-vuetify/components/date-period/u-v-date-period-box.vue";
...
const searchAction = (function () {
  ...
  async function loadSearchedData() {
    ...
    chartAction.chartList.value = (
      await service.getListHhChart(searchAction.searchParam)
    ).body;
  }
...
const searchPeriod = computed({
  get: () => {
    return [
      searchAction.searchParam.searchSrtDttm,
      searchAction.searchParam.searchEndDttm,
    ];
  },
  set: (v) => {
    useLogger().debug("", v);
...
const gridAction = (function () {
  ...
  function renderUsetime(data: LoginHist) {
    if (data.loginDttm && data.logotDttm) {
      const milliSec = $ustra.utils.date.getMilliDuration(
        new Date(data.loginDttm),
        new Date(data.logotDttm)
      );
