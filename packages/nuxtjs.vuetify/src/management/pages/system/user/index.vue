      ...
      <UFieldSet>
        <UFieldRow>
          ...
          <UField label="상태" :totalWidth="300">
            <UVCodeComboBox
              grpCd="USR_STT_CD"
              v-model="searchActions.criteria.usrSttCd"
              displayNullText="전체"
            />
          </UField>
              ...
              <UFieldSet>
                    ...
                    <VTextField
                      ...
                      :validation="{
                        rules: [
                          {
                            type: 'custom',
                            handler: formActions.validateId,
                            delay: 200,
                          },
                        ],
                      }"
                ...
                <UFieldRow>
                  <UField label="사번">
                    <VTextField
                      :isRequired="false"
                      v-model="formActions.inputData.empNo"
                    />
                  </UField>
                </UFieldRow>
                ...
                <UFieldRow>
                  <UField label="부서명">
                    <VTextField
                      :isRequired="false"
                      v-model="formActions.inputData.deptNm"
                    />
                  </UField>
                </UFieldRow>
                ...
                <UFieldRow>
                  <UField label="회사명">
                    <UVCodeComboBox
                      :isRequired="false"
                      v-model="formActions.inputData.orgCd"
                      grpCd="ORG_CD"
                      displayNullText="없음."
                    />
                  </UField>
                ...
                >
                  ...
                  >
                    <VTextField
                      :isReadOnly="true"
                      :isRequired="false"
                      v-model="formActions.inputData.pwd"
                    />
                  </UField>
                  <UField
                    :blank="
                  ...
                  >
                    ...
                    >
                      <VBtn
                        ...
                        @click="
                          () => {
                            formActions.inputData.pwd = $ustra.utils.system
                              .uuidBase62()
                              .substring(0, 8);
                            formActions.inputData.pwdResetYn = 'Y';
                          }
                ...
                <UFieldRow>
                  <UField label="구분" required>
                    <UVCodeComboBox
                      v-model="formActions.inputData.useDvCd"
                      grpCd="USE_DV_CD"
                    />
                  </UField>
                </UFieldRow>
                ...
                <UFieldRow>
                  <UField label="상태" required>
                    <UVCodeComboBox
                      v-model="formActions.inputData.usrSttCd"
                      grpCd="USR_STT_CD"
                    />
                  </UField>
                </UFieldRow>
                ...
                <UFieldRow>
                  <UField label="이메일" required>
                    <VTextField
                      v-model="formActions.inputData.email"
                ...
                <UFieldRow>
                  <UField label="사용기간">
                    <UVDatePeriodBox v-model="usePeriod" />
                  </UField>
                </UFieldRow>
                ...
                <UFieldRow>
                  <UField label="비고">
                    <VTextField
                      type="textarea"
                      v-model="formActions.inputData.rmk"
                    >
                    </VTextField>
                  </UField>
                </UFieldRow>
              ...
              <UButtonBox right top>
                <VBtn
                  ...
                  v-if="
                    formActions.isNew.value ||
                    (!formActions.isNew.value &&
                      authActions.hasApprovalStatus.value)
                  "
                />
                <VBtn
                  text="삭제"
                ...
                <ApprovalPopup
                  ...
                  :ref="
                    (popup) => {
                      approvalActions.popup.value = popup as InstanceType<
                        typeof ApprovalPopup
                      >;
                    }
                  "
                ></ApprovalPopup>
...
const usePeriod = computed({
  get: () => {
    return [formActions.inputData.useSrtDt, formActions.inputData.useEndDt];
  },
  set: (v) => {
    const period = v;
    if (period.length !== 2) {
    ...
    formActions.inputData.useSrtDt = period[0];
    formActions.inputData.useEndDt = period[1];
  },
});
...
const formActions = (() => {
  ...
  async function remove() {
    if (
      await confirm(
        "삭제 후 사용자는 시스템을 사용할 수 없으며, 모든 권한은 제거됩니다.\n삭제하시겠습니까?"
      )
    ) {
...
const authActions = await (async () => {
  ...
  const canUpdateAuthWhenEditUser =
    $ustra.management.serverAppProps.canUpdateAuthWhenEditUser;

  // 신규로 배정 가능한 권한 그룹 목록
  const newAuthorityGroups: AuthGroup[] = reactive(
    canUpdateAuthWhenEditUser
      ? await authGroupService.getRequestableGroupsOfUser(
          $ustra.management.currentSystemCode,
          null
        )
      : []
  );
  ...
  async function loadHasApprovalStatus(usrId: string) {
    hasApprovalStatus.value = await authService.hasApprovalAuth(
  ...
  const selectableAuthGroups = computed(() => {
    return formActions.isNew.value
      ? newAuthorityGroups
      : userAuthorityGroups.value;
  });
  ...
  return {
    canUpdateAuthWhenEditUser,
    newAuthorityGroups,
    loadUserAuthorityGroups,
    selectableAuthGroups,
    hasApprovalStatus,
    loadHasApprovalStatus,
  };
})();
