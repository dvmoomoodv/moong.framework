  ...
  <VCard>
    <template #text>
      ...
      <UButtonBox right top>
        <VBtn color="primary" :width="80" @click="save" v-if="!!user.sub">
          변경
        </VBtn>
      </UButtonBox>
    </template>
...
import { computed, reactive, ref, shallowRef } from "#moong/nuxt";
import { useUstraLoginService } from "#moong/nuxt/management";
import { useUstraManagementUser } from "#moong/nuxt/management/composables";
...
const passwordGuide = computed(() => {
  return $ustra.env.appProps.nuxt.management.security.password
...
async function validateNewPassword() {
  ...
  const passwordValidResult = await service.validPassword(
