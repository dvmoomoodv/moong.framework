<template>
  <v-row class="mb-10">
    ...
    <VCol cols="12">
      <span class="d-block text-caption">- Vuetify 참고 링크 : <NuxtLink to="https://vuetifyjs.com/">Link</NuxtLink></span>
      <span class="d-block text-caption">- Ckeditor 참고 링크 : <NuxtLink to="https://ckeditor.com/">Link</NuxtLink></span>
      <span class="d-block text-caption">- Vue Treeview 참고 링크 : <NuxtLink to="https://n00ts.github.io/vue3-treeview/">Link</NuxtLink></span>
      <span class="d-block text-caption">- Apexchart 참고 링크 : <NuxtLink to="https://apexcharts.com/">Link</NuxtLink></span>
      <span class="d-block text-caption">- Maska 참고 링크 : <NuxtLink to="https://beholdr.github.io/maska/#/">Link</NuxtLink></span>
      <span class="d-block text-caption">
        - Draggable & Resizable 참고 링크 :
        <NuxtLink to="https://mauricius.github.io/vue-draggable-resizable/#/story/stories-basic-cancelhandle-story-vue?variantId=_default"
          >Link</NuxtLink
        ></span
      >
      <span class="d-block text-caption">
    ...
    <VCol cols="12" align="left">
      ...
      <span class="text-h8 font-weight-black">리얼그리드 메뉴 작업전 샘플 메인페이지에서 작업합니다.</span><br />
    ...
    <VCol cols="12" align="left">
      ...
      <VBtn @click="changeSelectionType('singleRow')">selectionType single row</VBtn>
      <VBtn @click="changeSelectionType('singleColumn')">selectionType single column</VBtn>
...
import { columns, fields, rows } from '@/pages/vuetify/samples/_data/realgrid-data.ts'
...
onMounted(async () => {
  const { gridView1, dataProvider1 } = grid1.value.initRG(fields, columns, rows, {})

  const { gridView2, dataProvider2 } = grid2.value.initRG(fields, columns, rows, {})

  const { gridView3, dataProvider3 } = grid3.value.initRG(fields, columns, rows, {})
 ...
 */
const changeSelectionType = (selectionType: 'block' | 'rows' | 'columns' | 'singleRow' | 'singleColumn' | 'none') => {
