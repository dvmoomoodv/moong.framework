...
import { ref, onMounted, useUstraEnv, computed } from "#moong/nuxt";
...
const menus = computed(() => {
  ...
  if (useUstraEnv().appProps.nuxt.vuetify.samples.additionalMenus) {
    const additionalMenus = $ustra.utils.core.deepMergeArrayConcat(
      [],
      useUstraEnv().appProps.nuxt.vuetify.samples.additionalMenus
    );
  ...
  $ustra.utils.model.reclusiveEach(menus, "items", (menu, parents) => {
    ...
    if (parents) {
      parents
        .reverse()
        .forEach((p) => (menu._fullTitle = p.title + " / " + menu._fullTitle));
    }
  });
...
onMounted(() => {
  ...
  let selectedMenu = $ustra.utils.model
    .flatReclusiveArray(menus.value, "items", false, true)
    .find((item) => {
      if (!item.path) {
        return false;
      }

      const path = $ustra.utils.path.withoutExt(
        $ustra.utils.path.join(
