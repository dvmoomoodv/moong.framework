...
<template>
  ...
  <UCheckGroupBox
    v-model="options.caseSensitiveSearch"
    :itemsSource="[{ text: 'caseSensitiveSearch' }]"
  />
  <UCheckGroupBox
    v-model="options.beginsWithSearch"
    :itemsSource="[{ text: 'beginsWithSearch' }]"
  />
  <UCheckGroupBox
    v-model="options.isAnimated"
    :itemsSource="[{ text: 'isAnimated' }]"
  />
  <br />
  <span
    >delay: <WjInputNumber v-model="options.delay" style="width: 100px" />
  </span>
  <span class="ml-3">
    maxItems:
