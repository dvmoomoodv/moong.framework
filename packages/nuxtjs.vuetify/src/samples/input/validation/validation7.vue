<template>
  <v-card>
    ...
    <template #text>
      checkValidationOnUpdate 속성은 입력 영역의 값이 변경될 경우, validation
      처리를 자동으로 처리할지 여부를 설정하는 속성이다. 기본 값은 true이며,
      false로 설정할 경우, 입력 영역에 값이 변경하더라도 validation 처리를
      수행하지 않는다.
      <UValidationGroup
        ref="validationGroup"
      ...
      <UButton
        :text="
          checkValidationOnUpdate ? '입력비감지로 전환' : '입력감지로 전환'
        "
