...
<script setup lang="ts">
  ...
  import { ref, reactive, computed } from "#moong/nuxt";
  import { useWijmo } from "#moong/nuxt-wijmo/composables/wijmo";
  import { useWijmoChart } from "#moong/nuxt-wijmo/composables/chart";

  const showMarker = ref(false);
  const palette = computed(() => {
    let palettes = Object.keys(useWijmoChart().Palettes).filter((prop) =>
      useWijmo().isArray(useWijmoChart().Palettes[prop])
    );
  ...
  const onChartInitialized = (sender) => {
    sender.hostElement.addEventListener(
      "mouseenter",
